...
import org.baiHoo.db.formatUtils.FormatUtils;
...
public class ExcelTemplate {
							...
							if (value != null) {
								...
								if (value.startsWith(FORMULA_FLAG)) {
									...
									HSSFCell startC = sheet.getRow(
											start_row - 1).getCell((short) start_col);
	...
	*/
	 ...
	 *            HSSFSheet
	*/
	private void generateContentDatas(List<Object> resultset,
	 ...
	 * @throws Exception
	*/
	public void generate(List<Object> resultset) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void generate(List<Object> resultset, Object exportInfo)
			throws Exception {
	...
	*/
	private void generatePagination(HSSFWorkbook wb, HSSFSheet sourceSheet,
			List<Object> resultset, Object exportInfo) throws Exception {
		...
		while (current_page < count) {
			List<Object> newList = resultset.subList(current_page * maxrow,
	...
	*/
	private void generatePaginationSheet(HSSFWorkbook wb,
			HSSFSheet sourceSheet, List<Object> resultset,
			Object exportInfo) throws Exception {
		int startPosition = startRow;
		int count = resultset.size() / maxrow;
		int num = resultset.size() % maxrow;
		if (num > 0) {
		...
		if (count > 1) {
			for (int i = 0; i < wb.getNumberOfSheets(); i++) {
				...
				List<Object> newList = resultset.subList(i * maxrow, maxrow
						* (i + 1));
	...
	*/
	...
	public int importData(String targetFile, String entityName, int submitCount)throws Exception {
		...
		for (int sheetCount = 0; sheetCount < wb.getNumberOfSheets(); sheetCount++) {
			...
			for (int rowCount = startRow + 2; rowCount < rows; rowCount++) {
				...
				DataObject importEntity = new DataObject(entityName);
				...
				for (int cellCount = 0; cellCount < fieldNames.length; cellCount++) {
					...
					Type type =null;
					if(importEntity.isOraProperty(propertyName)){
						 type = importEntity.getPropertyTypeByOra(propertyName);
					}else {
						type = importEntity.getPropertyType(propertyName);
					}
					//-------------------start------------
					//baiHoo.chen -------------- addedd
					if(dictTypeId!=""){
					...
					//baiHoo.chen -------------- added
					//-------------------end------------
					if ("java.lang.Integer".equals(type.getTypeName()) ||  "int".equals(type.getTypeName())) {
						// 防止可能出现的Excel表格读取自动加.号
						if (value.indexOf(".") != -1)
							value = value.substring(0, value.indexOf("."));
							importEntity.set(propertyName, new Integer(Integer.parseInt(value)));
					} else if ("java.lang.Boolean".equals(type.getTypeName())  || "boolean".equals(type.getTypeName())) {
						importEntity.set(propertyName, Boolean.valueOf(value));
					} else if ("java.lang.Float".equals(type.getTypeName())  || "float".equals(type.getTypeName())) {
						importEntity.set(propertyName, new Float(Float.parseFloat(value)));
					} else if ("java.lang.Long".equals(type.getTypeName())  || "Long".equals(type.getTypeName())) {
						if (value.indexOf(".") != -1)
							value = value.substring(0, value.indexOf("."));
						importEntity.set(propertyName, new Long(Long.parseLong(value)));
					}  else if ("java.util.Date".equals(type.getTypeName())) {
						importEntity.set(propertyName, FormatUtils.formatDate(value));
					} else {
				...
				if(fieldCapacity>0){
					...
					if(dataEntity!=null){
						...
						dataEntity.set("lastUpdateDate", new Date());
						dataEntity.set("lastUpdateIp","o.o.o.o");
						DatabaseUtils.updateEntity("default", dataEntity);
					}
				}
				//baiHoo.chen -------------- added
				//-------------------end------------
				importEntity.set("creationDate", new Date());
				importEntity.set("lastUpdateDate", new Date());
				importEntity.set("lastUpdateIp","o.o.o.o");
	 ...
	 * @return
	*/
	private DataObject expandEntityByTemplate(String entityName , String[] fields , String[] fieldValues) throws Exception{
		
		DataObject dataEntity = new DataObject(Class.forName(entityName));
		for(int i = 0; fields!=null && i<fields.length;i++){
			if(fields[i]!=null && fieldValues[i] !=null && fields[i]!="" && fieldValues[i] !=""){
				dataEntity.set(fields[i], fieldValues[i]);
			}
		}
		DataObject resutl = DatabaseUtils.expandEntity("default",dataEntity);
		if(resutl !=null){
			return resutl;
		}
		return null;
	 ...
	 * @throws Exception 
	*/
	private String getValueByDataDictIsKey(String propertyValue, String dictTypeId) throws Exception{
		DataObject dictEntity = new DataObject("com.avicit.fdmp.primeton.newdataset.EosDictEntry");
		dictEntity.set("dicttypeid", dictTypeId);
		dictEntity.set("dictname", propertyValue);
		DataObject resutl = DatabaseUtils.expandEntity("default",dictEntity);
		String value="";
		if(resutl !=null){
			value = (String) dictEntity.get("dictid");
		}
		return value;
	}
	...
	*/
	public void generateBySheet(Map<String, List> map, Object exportInfo)
			throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(
				templateFile));
		HSSFWorkbook wb = new HSSFWorkbook(fs);
		...
		for (String key : map.keySet()) {
			...
			List<Object> resultset = map.get(key);
			HSSFSheet sheet = wb.getSheetAt(i++);
			initialize(sheet);
			if (startRow == -1)
				return;
