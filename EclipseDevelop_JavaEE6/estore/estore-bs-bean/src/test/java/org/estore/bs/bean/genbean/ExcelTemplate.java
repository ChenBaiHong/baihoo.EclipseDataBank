...
public class ExcelTemplate {
	...
	public int importData(String targetFile, String entityName, int submitCount)throws Exception {
		...
		for (int sheetCount = 0; sheetCount < wb.getNumberOfSheets(); sheetCount++) {
			...
			for (int rowCount = startRow + 2; rowCount < rows; rowCount++) {
				...
				//-------------------start------------
				//baiHoo.chen -------------- added
				//针对数据增量导入，特殊字段去唯一性校验的
				String[] allCellFields = new String[fieldNames.length];
				String[] specialFields = new String[2];
				...
				for (int cellCount = 0; cellCount < fieldNames.length; cellCount++) {
					...
					//baiHoo.chen -------------- added
					String dictTypeId="";
					if(isRegExPropertyName(propertyName)){
						String[] arrays = regExSplitPropertyName(propertyName);
						propertyName = arrays[0];
						dictTypeId= arrays[1];
						if(dictTypeId.trim().equals("**")||dictTypeId.trim().equals("*")){
							if(fieldCapacity>=specialFields.length){
								String[] arr= new String[fieldCapacity*2]; 
								System.arraycopy(specialFields, 0, arr, 0, specialFields.length);
								specialFields = arr;
							}
							specialFields[fieldCapacity++]=propertyName;
						}
					}
					allCellFields[cellCount]=propertyName;
					//baiHoo.chen -------------- added
					//-------------------end------------
					...
					if(dictTypeId!=""){
						if(dictTypeId.trim().equals("**")||dictTypeId.trim().equals("*")){
							if(valueCapacity>=fieldValues.length){
								String[] arr= new String[valueCapacity*2]; 
								System.arraycopy(fieldValues, 0, arr, 0, fieldValues.length);
								fieldValues = arr;
							}
							fieldValues[valueCapacity++]=value;
						}else{
							value=getValueByDataDictIsKey(value, dictTypeId);
						}
					}
					//baiHoo.chen -------------- added
					//-------------------end------------
				...
				//-------------------start------------
				//baiHoo.chen -------------- addedd
				DataObject dataEntity=null;
				if(fieldCapacity>0){
					dataEntity = expandEntityByTemplate(entityName,specialFields, fieldValues);
					if(dataEntity!=null){
						for(int i = 0 ; i<allCellFields.length ; i++){
							if(dataEntity.get(allCellFields[i])!=null && importEntity.get(allCellFields[i])!=null){
								if(dataEntity.get(allCellFields[i]).equals(importEntity.get(allCellFields[i]))){
									continue;
								}else{
									String va = importEntity.get(allCellFields[i])+"";
									if(!va.trim().equals("")){
										dataEntity.set(allCellFields[i], importEntity.get(allCellFields[i]));
									}
								}
							}else if(dataEntity.get(allCellFields[i])== null && importEntity.get(allCellFields[i])!=null){
								String va = importEntity.get(allCellFields[i])+"";
								if(!va.trim().equals("")){
									dataEntity.set(allCellFields[i], importEntity.get(allCellFields[i]));
								}
						...
						dataEntity.set("lastUpdatedBy",muo==null?"":(String)muo.get("userObject/userId"));
						dataEntity.set("lastUpdateDate", DateUtil.parse(DateUtil.format(new java.util.Date(), "yyyy-MM-dd HH:mm:ss"),"yyyy-MM-dd HH:mm:ss"));
						dataEntity.set("lastUpdateIp",muo==null?"":(String)muo.get("userObject/userRemoteIP"));
						DatabaseUtil.updateEntity("default", dataEntity);
					}
				}
				//baiHoo.chen -------------- added
				//-------------------end------------
				importEntity.set("createdBy",muo==null?"":(String)muo.get("userObject/userId"));
				importEntity.set("creationDate", DateUtil.parse(DateUtil.format(new java.util.Date(), "yyyy-MM-dd HH:mm:ss"),"yyyy-MM-dd HH:mm:ss"));
				importEntity.set("lastUpdatedBy",muo==null?"":(String)muo.get("userObject/userId"));
				importEntity.set("lastUpdateDate", DateUtil.parse(DateUtil.format(new java.util.Date(), "yyyy-MM-dd HH:mm:ss"),"yyyy-MM-dd HH:mm:ss"));
				importEntity.set("lastUpdateIp",muo==null?"":(String)muo.get("userObject/userRemoteIP"));
				if (dataObjects.size() < submitCount) {
					if (allowInsert && dataEntity==null){
	...
	private DataObject expandEntityByTemplate(String entityName , String[] fields , String[] fieldValues){
		DataObject dataEntity = DataObjectUtil.createDataObject(entityName);
		for(int i = 0; fields!=null && i<fields.length;i++){
			if(fields[i]!=null && fieldValues[i] !=null && fields[i]!="" && fieldValues[i] !=""){
				dataEntity.setString(fields[i], fieldValues[i]);
			}
		}
		int resutl = DatabaseUtil.expandEntityByTemplate("default", dataEntity, dataEntity);
		if(resutl>0){
			return dataEntity;
		}
		return null;
	...
	private String getValueByDataDictIsKey(String propertyValue, String dictTypeId){
		DataObject dictEntity = DataObjectUtil.createDataObject("com.avicit.fdmp.primeton.newdataset.EosDictEntry");
		dictEntity.setString("dicttypeid", dictTypeId);
		dictEntity.setString("dictname", propertyValue);
		int resutl = DatabaseUtil.expandEntityByTemplate("default", dictEntity, dictEntity);
		String value="";
		if(resutl>0){
			value = dictEntity.getString("dictid");
		}
		return value;
	}
	 ...
	 * @return
	 */
	private String[] regExSplitPropertyName(String propertyName){
	 ...
	 * @return
	 */
	private boolean isRegExPropertyName(String propertyName){
	...
	@SuppressWarnings("deprecation")
	public int importDataStoreIn(String targetFile, String entityName, int submitCount)throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(targetFile));
		HSSFWorkbook wb = new HSSFWorkbook(fs);
		IMUODataContext muo = DataContextManager.current().getMUODataContext();
		for (int sheetCount = 0; sheetCount < wb.getNumberOfSheets(); sheetCount++) {
			...
			for (int rowCount = startRow + 2; rowCount < rows; rowCount++) {
				...
				for (int cellCount = 0; cellCount < fieldNames.length; cellCount++) {
					...
					if("productCode".equals(propertyName)){
						value = selectProducrName(value);
					}
					if("warehoseId".equals(propertyName)){
						value = selectWarehoseName(value);
					}
					if("classifyId".equals(propertyName)){
						value = selectClassifyName(value);
					}
					TypeReference typeReference = (TypeReference) importEntity.getType().getProperty(propertyName).getType();
					Type propertyType = typeReference.getActualType();
					//-------------------start------------
	...
	public String selectProducrName(String producname){
		...
		try{
			...
			//conn = DriverManager.getConnection("jdbc:oracle:thin:@172.18.27.194:1521:orcl","fdmp","fdmp_132CF");
			//conn1 = DriverManager.getConnection("jdbc:oracle:thin:@172.18.27.194:1521:orcl","fdmp","fdmp_132CF");
			conn = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.99:1521:fdmp","fdmp","cape");
			conn1 = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.99:1521:fdmp","fdmp","cape");
			sta = conn.createStatement();
			String sql = "select distinct product_code from fdmp_planetree where product_name=\'"+producname+"\'";
			res = sta.executeQuery(sql);
			while(res.next()){
				pcode = res.getString(1);
			}		
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	public String selectWarehoseName(String wareName){
		Connection conn = null;
		Connection conn1 = null;
		Statement sta = null;
		ResultSet res = null;
		...
		try{
			...
			conn1 = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.99:1521:fdmp","fdmp","cape");
			sta = conn.createStatement();
			String sql = "select t.warehose_id from FDMP_WAREHOUSE t where t.fdmp_warehouse_full_name=\'"+wareName+"\'";
			res = sta.executeQuery(sql);
			while(res.next()){
				pcode = res.getString(1);
			}		
	...
	public String selectClassifyName(String classifyName){
		...
		try{
			...
			conn1 = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.99:1521:fdmp","fdmp","cape");
			sta = conn.createStatement();
			String sql = "select distinct t.classify_id from FDMP_STORE_CLASSIFY t where t.classify_name=\'"+classifyName+"\'";
			res = sta.executeQuery(sql);
			while(res.next()){
				pcode = res.getString(1);
			}		
	...
	public void generateBySheet(Map<String, List> map, DataObject exportInfo)
			throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(
				templateFile));
		HSSFWorkbook wb = new HSSFWorkbook(fs);

		// List<DataObject>dedicated= map.get("dedicated");
		// List<DataObject>maintain= map.get("maintain");
		// List<DataObject>alter= map.get("alter");
		int i = 0;
		for (String key : map.keySet()) {
			// System.out.println("key= "+ key);
			List<DataObject> resultset = map.get(key);
			HSSFSheet sheet = wb.getSheetAt(i++);
			initialize(sheet);
			if (startRow == -1)
				return;
	...
	public static void main(String args[]){
		try{
			DataObject dataEntity = DataObjectUtil.createDataObject("com.avicit.fdmp.depot.storeIn.fdmpstorein.FdmpStoreIn");
