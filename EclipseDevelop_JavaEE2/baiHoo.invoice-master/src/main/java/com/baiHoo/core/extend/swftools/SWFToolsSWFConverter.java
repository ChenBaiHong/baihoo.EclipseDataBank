...
/**
 * 
 *<p>Title: SWFToolsSWFConverter</p>
 *<p>Description: 
 *
 *
 *
...
public class SWFToolsSWFConverter implements SWFConverter {
	...
	public void convert2SWF(String inputFile, String swfFile, String extend) {
		...
		if (!pdfFile.exists()) {
			 com.baiHoo.core.util.LogUtil.info("PDF文件不存在！");
			return;
		}
	
		if (outFile.exists()) {
			 com.baiHoo.core.util.LogUtil.info("SWF文件已存在！");
			return;
		}
...
//				+ "\" -o " + swfFile + " -s languagedir=D:\\xpdf-chinese-simplified -T 9 -f";
		try {
			...
			try {
				process.waitFor();
				com.baiHoo.core.util.LogUtil.info("时间-------"+process.waitFor());
